...
**Status:** authoritative for Mira's Phase 1 mapper output and Phase 3 loader contract.
...
- Bumping any schema requires a new subfolder in `mira/midlayer/vN/`. The table family stays the same; consumers pin `schema_version` per row or per load batch.
